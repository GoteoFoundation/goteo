<!-- POST PROCESSING THIS JAVASCRIPT BY GRUNT -->
<!-- build:js assets/js/map.js -->
<script src="<?= SRC_URL ?>/assets/vendor/leaflet/dist/leaflet.js"></script>
<script src="<?= SRC_URL ?>/assets/vendor/leaflet.markercluster/dist/leaflet.markercluster.js"></script>
<script src="<?= SRC_URL ?>/assets/vendor/leaflet.fullscreen/Control.FullScreen.js"></script>
<script type="text/javascript" src="<?= $this->asset('js/map/map.js') ?>"></script>
<!-- endbuild -->
