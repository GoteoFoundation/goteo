...
<div class="widget report">
    ...
    <table>
        ...
        <tr>
            <td>-&nbsp;&nbsp;&nbsp;&nbsp;Máximo mostrado en el termómetro de Goteo.org al cerrar la campaña (<?php echo ($project->success) ? date('d/m/Y', strtotime($project->success)) : 'fecha'; ?>): <strong><?php echo \amount_format($sumData['shown'], 2); ?></strong>
                <br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<em>(Puede que no se haya llegado nunca a esta cifra si han devuelto aportes antes del cierre de campaña)</em>
            </td>
        </tr>
        <tr>
        ...
        <?php if ($matchers) : ?>
            <?php foreach ($matchers as $matcher): ?>
                <tr>
                    <td>-&nbsp;&nbsp;&nbsp;&nbsp;Riego obtenido del <strong>Canal <?= $matcher->name ?></strong>: <strong><?= amount_format($matchers_amounts[$matcher->id])  ?></strong> (A transferir de forma directa)</td>
                </tr>
                <tr>
                    <td>
    ...
    <table>
        ...
        <?php if ($admin) : ?>
            <!--
        <tr>
            <td>-&nbsp;&nbsp;&nbsp;&nbsp;Desglose calculo Comisión del 8&#37; de Goteo.org: <?php $aportes = $Data['tpv']['total']['amount'] + $Data['paypal']['total']['amount'] + $Data['cash']['total']['amount'];
                                                                                            $comisionaportes = $aportes * 0.08;
                                                                                            $comisionriego = $Data['drop']['total']['amount'] * 0.08;
                                                                                            echo "Aportes usuarios: {$Data['tpv']['total']['amount']}(tpv) + {$Data['paypal']['total']['amount']}(paypal) + {$Data['cash']['total']['amount']}(cash) = {$aportes} -> {$comisionaportes} Capital Riego: {$Data['drop']['total']['amount']} -> {$comisionriego}"; ?></td>
        </tr>
        -->
        <?php endif; ?>
        <tr>
    ...
    <?php if (!empty($Data['issues'])) : ?>
        ...
        <table>
            ...
            ?>
                <tr>
                    <?php if ($admin) : ?>
                        <td><?php echo '<a href="/admin/accounts/details/' . $issue->invest . '" target="_blank"' . $warst . '>[Ir al aporte]</a> Usuario <a href="/admin/users/manage/' . $issue->user . '" target="_blank">' . $issue->userName . '</a> [<a href="mailto:' . $issue->userEmail . '">' . $issue->userEmail . '</a>], ' . $issue->statusName . ', ' . $issue->amount . ' euros.';
                            if (!empty($warst)) echo '  (Aporte: ' . $issue->invest . ')'; ?></td>
                    <?php else: ?>
                        <td<?php echo $warst; ?>>Usuario/a <?php echo $issue->userName; ?>, <?php echo $issue->statusName; ?>, <?php echo $issue->amount . ' ss.';
                                                                                                                                if (!empty($warst)) echo '  (Aporte: ' . $issue->invest . ')'; ?></td>
                        <?php endif; ?>
                </tr>
            <?php endforeach; ?>
