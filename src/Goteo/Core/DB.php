...
class DB extends PDO {
	...
	public function __construct(Cacher $cache = null, $debug = false) {
		...
		try {
			...
			parent::__construct(
                ...
                $username,
                $password,
                // Avoid strict mysql configurations (sorry we're not there yet, so many old queries)
	...
	public function getQueryStats() {
		$ret = array();
		$ret['replica']['non-cached'] = CacheStatement::$query_stats['replica'][0] . ' ( ' . CacheStatement::$query_stats['replica'][2] . 's)';
		$ret['replica']['cached'] = CacheStatement::$query_stats['replica'][1];
		$ret['master']['non-cached'] = CacheStatement::$query_stats['master'][0] . ' ( ' . CacheStatement::$query_stats['master'][2] . 's)';
		$ret['master']['cached'] = CacheStatement::$query_stats['master'][1];
		$ret['sql_replica']['non-cached'] = CacheStatement::$queries['replica'][0];
		$ret['sql_replica']['cached'] = CacheStatement::$queries['replica'][1];
		$ret['sql_master']['non-cached'] = CacheStatement::$queries['master'][0];
		$ret['sql_master']['cached'] = CacheStatement::$queries['master'][1];
