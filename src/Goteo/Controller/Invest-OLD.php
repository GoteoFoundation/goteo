...
class Invest extends \Goteo\Core\Controller
{
    ...
    public function index($project = null)
    {
        ...
        if ($projectData->status != Model\Project::STATUS_IN_CAMPAIGN) {
    ...
    public function confirmed($project = null, $id = null, $reward = null)
    {
        ...
        */
        ...
        if ($invest->method == 'paypal') {
            ...
            $log->populate(
                'Aporte PayPal', '/admin/invests',
                \vsprintf(
                    "%s ha aportado %s al proyecto %s mediante PayPal",
                    array(
        ...
        if ($invest->method == 'pool') {
            ...
            $log->setTarget($projectData->id);
            $log->populate(
                'Aporte Monedero', '/admin/invests',
                \vsprintf(
                    "%s ha aportado %s al proyecto %s mediante Monedero",
                    array(
                        Feed::item('user', $user->name, $user->id),
                        Feed::item('money', $invest->amount.' &euro;'),
                        Feed::item('project', $projectData->name, $projectData->id))
        ...
        if (!empty($invest->droped) && $drop instanceof Model\Invest && is_object($callData)) {
            ...
            $log->setTarget($projectData->id);
            $log->populate(
                'Aporte riego '.$drop->method, '/admin/invests',
                \vsprintf(
                    "%s ha aportado %s de %s al proyecto %s a través de la campaña %s", array(
                ...
                // evento público
                $log->populate(
                    $callData->user->name, '/user/profile/'.$callData->user->id,
                    Text::html(
                        'feed-invest',
                        Feed::item('money', $drop->amount.' &euro;')
                        . ' de '
