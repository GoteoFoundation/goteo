...
class ProjectController extends \Goteo\Core\Controller {
	...
	public function editAction($pid, $step = 'userProfile', Request $request) {
        ...
        if ($request->isMethod('post')) {
            ...
            if (!empty($errors['alert'])) {
                $viewData['alert'] = $errors['alert'];
                Application\Message::error(is_array($errors['alert']) ? implode('<br />', $errors['alert']) : $errors['alert']);
            }
    ...
    private function process_userProfile($project, &$errors) {
        ...
        // Google analytics
        $project->analytics_id = $_POST['analytics_id'];

        //Facebook pixel

        $project->facebook_pixel = $_POST['facebook_pixel'];
    ...
    private function process_overview($project, &$errors) {
        ...
        foreach ($fields as $field) {
            if($field === 'lang' && Project::isTranslated($project->id, $_POST[$field])) {
                $errors['alert'] = sprintf(Text::get('project-error-main-lang'), $_POST[$field], $_POST[$field]);
                continue;
            }
            $project->$field = $_POST[$field];
        }
        ...
        if($guarda = @array_diff($viene, $tiene)) {
