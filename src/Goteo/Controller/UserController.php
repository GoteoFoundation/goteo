...
class UserController extends \Goteo\Core\Controller {
    ...
    public function loginAction($username = '', Request $request) {
        ...
        if ($request->request->has('amount')) {
            Session::store('invest-amount', $request->request->get('amount'));
            $msg = Text::get('user-login-required-login');
            $msg .= ($request->request->get('amount') ? '. ' . Text::get('invest-alert-investing') . ' ' . $request->request->get('amount') . Session::get('currency') : '');
