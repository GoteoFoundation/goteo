        ...
        public static function process ($action = 'list', $id = null, $filters = array()) {
                ...
                if (isset($_POST['save-dates'])) {
                    ...
                    if ($_POST['proceed'] == 'rebase' && !empty($_POST['newid'])) {
                        ...
                        if ($projData->status >= 3 && $_POST['force'] != 1) {
                            Message::Error('El proyecto no est谩 ni en Edici贸n ni en Revisi贸n, no se modifica nada.');
                            throw new Redirection('/admin/projects/rebase/'.$id);
                        }
            ...
            switch ($action) {
                case 'review':
                    ...
                    break;
                case 'publish':
                    // poner un proyecto en campa馻
                    if ($project->publish($errors)) {
                        $log_text = 'El admin %s ha pasado el proyecto %s al estado <span class="red">en Campa馻</span>';
                    } else {
                        $log_text = 'Al admin %s le ha fallado al pasar el proyecto %s al estado <span class="red">en Campa馻</span>';
                    }
                    break;
                case 'cancel':
            ...
            if ($action == 'reject') {
                if (empty($project)) {
                    ...
                    if ($mailHandler->send()) {
                        Message::Info('Se ha enviado un email a <strong>'.$project->user->name.'</strong> a la direcci贸n <strong>'.$project->user->email.'</strong>');
                    } else {
                        Message::Error('Ha fallado al enviar el mail a <strong>'.$project->user->name.'</strong> a la direcci贸n <strong>'.$project->user->email.'</strong>');
                    }
                    unset($mailHandler);
                }
