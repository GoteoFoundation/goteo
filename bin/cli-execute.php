...
define('LANG', 'es');
// while debugging, this won't affect database in any way
$DEBUG = true;
if ($argv[1] !== '--update') {
    echo "Debugging!. Use the --update modified to actually update the database\n";
    $DEBUG = false;
}
...
function project_fail($project, $per_amount)
{
    ...
    if (!$DEBUG) {
        ...
        // @FIXME : verificar si esto se puede hacer en cli mode
...
function cancel_payment($invest, $project, $userData)
{
    ...
    echo "Aporte " . $invest->id . " cancelado por proyecto caducado.\n";
    $invest->setStatus('4');
 ...
 *
 */
function process_project($project, $debug = false)
{
    global $DEBUG;
...

    {
        ...
        echo "Mínimo: " . $project->mincost . " &euro; \n";
        echo "Obtenido: " . $project->amount . " &euro;\n";
