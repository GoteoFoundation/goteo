          ...
          for(i in data['items']) {
            ...
            var item_date=moment(new Date(item_start)).format("MM-DD-YYYY");
              ...
              if(event_date==item_date)
              {
                ...
                if(num>1)
                {
                  other_events=other_events+'<a class="other-event" href="/calendar#'+item.id+'" ><span class="event-hour hour-other-event">'+event_start+' - '+event_end+'</span>'+item.summary.replace(reg,'')+'</a>';
